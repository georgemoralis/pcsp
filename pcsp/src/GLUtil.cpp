...
#define ERROR_LOG(a, ...) printf(__VA_ARGS__);
#define NOTICE_LOG(a, ...) printf(__VA_ARGS__);
...
void UpdateFPSDisplay(const char *text)
{
    char temp[512];
#ifndef SVN_REV_STR
#define SVN_REV_STR "unknown rev"
#endif
...
const char *cFSResolution = "1024x768";
const char *cInternalRes = "480x272";

// Create rendering window.
//		Call browser: Core.cpp:EmuThread() > main.cpp:Video_Initialize()
bool OpenGL_Create(bool fullscreen, int _iwidth, int _iheight)
{
#if !defined(HAVE_X11) || !HAVE_X11
    ...
    if (fullscreen)
    {
        if (strlen(cFSResolution) > 1)
        {
            sscanf(cFSResolution, "%dx%d", &_twidth, &_theight);
        }
    ...
    {
        if (strlen(cInternalRes) > 1)
        {
            sscanf(cInternalRes, "%dx%d", &_twidth, &_theight);
        }
  ...
  int attrListDbl[] = {GLX_RGBA, GLX_DOUBLEBUFFER,
                       ...
                       GLX_BLUE_SIZE, 8,
                       GLX_DEPTH_SIZE, 24,
                       GLX_SAMPLE_BUFFERS_ARB, iMultisampleMode, GLX_SAMPLES_ARB, 1, None };
  GLWin.dpy = XOpenDisplay(0);
//  g_VideoInitialize.pWindowHandle = (void *)GLWin.dpy;
  GLWin.screen = DefaultScreen(GLWin.dpy);

  // Fullscreen option.
  GLWin.fs = fullscreen; //Set to setting in Options
  ...
  if (strlen(cFSResolution) > 1)
    sscanf(cFSResolution, "%dx%d", &GLWin.fullWidth, &GLWin.fullHeight);
  ...
  if (strlen(cInternalRes) > 1)
    sscanf(cInternalRes, "%dx%d", &GLWin.winWidth, &GLWin.winHeight);
  else // No Window resolution set, fall back to default
  {
