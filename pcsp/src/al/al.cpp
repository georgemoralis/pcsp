...
#include "al_disassembler.h"
#include "al_interpreter.h"

#undef IDEF

#define IDEF(nam, fmt, msk, sig, inf) void AllegrexInstruction_##nam::Interpret(AlContext &m_thread_context, u32 opcode) { FastInterpret(m_thread_context, opcode); }
#include "al.def"
#undef IDEF

#define IDEF(nam, fmt, msk, sig, inf) __forceinline char const *AllegrexInstruction_##nam::FastOpcodeName() { return #nam; }
#include "al.def"
#undef IDEF

#define IDEF(nam, fmt, msk, sig, inf) insn_count_t AllegrexInstruction_##nam::count = insn_count_t();
#include "al.def"
#undef IDEF

insn_count_t AllegrexInstruction::dummy_count = insn_count_t();

namespace Allegrex
{
	insn_count_t all_insn_count = insn_count_t();
	insn_count_t cpu_insn_count = insn_count_t();
	insn_count_t mdu_insn_count = insn_count_t();
	insn_count_t lsu_insn_count = insn_count_t();
	insn_count_t bcu_insn_count = insn_count_t();
	insn_count_t fpu_insn_count = insn_count_t();
	insn_count_t vpu_insn_count = insn_count_t();
	insn_count_t unk_insn_count = insn_count_t();

	AllegrexInstructionUnknown &reserved_instruction = AllegrexInstructionUnknown::Self();

#define IDEF(nam, fmt, msk, sig, inf) AllegrexInstruction_##nam &nam = AllegrexInstruction_##nam ::Self();
#include "al.def"
#undef IDEF

	void reset_counts()
	{
#define IDEF(nam, fmt, msk, sig, inf) AllegrexInstruction_##nam::count = insn_count_t();
#include "al.def"
#undef IDEF
		all_insn_count = insn_count_t();
		cpu_insn_count = insn_count_t();
		mdu_insn_count = insn_count_t();
		lsu_insn_count = insn_count_t();
		bcu_insn_count = insn_count_t();
		fpu_insn_count = insn_count_t();
		vpu_insn_count = insn_count_t();
		unk_insn_count = insn_count_t();
	}
	...
	void update_counts()
	{
		...
		vpu_insn_count = insn_count_t();
		unk_insn_count = insn_count_t();
#define IDEF(nam, fmt, msk, sig, inf) \
		all_insn_count += AllegrexInstruction_##nam::count; \
		if ((inf) & IS_CPU) cpu_insn_count += AllegrexInstruction_##nam::count; \
		if ((inf) & IS_MDU) mdu_insn_count += AllegrexInstruction_##nam::count; \
		if ((inf) & IS_LSU) lsu_insn_count += AllegrexInstruction_##nam::count; \
		if ((inf) & IS_BCU) bcu_insn_count += AllegrexInstruction_##nam::count; \
		if ((inf) & IS_FPU) fpu_insn_count += AllegrexInstruction_##nam::count; \
		if (!(inf)) unk_insn_count += AllegrexInstruction_##nam::count; \

#include "al.def"
#undef IDEF
	}

	AllegrexInstruction *instruction_array[] =
	{
#define IDEF(nam, fmt, msk, sig, inf) &nam,
#include "al.def"
#undef IDEF
		0
	};
	...
	u64 Disassemble(u32 address, u32 opcode, char *line)
	{
		...
		char operands[32];
		char comment[256];
		AllegrexInstruction *insn = Allegrex::DecodeInstruction(opcode);
		insn->Disassemble(address, opcode, opcode_name, operands, comment);
		::sprintf(line, "%08X  %-15s %-32s ",opcode, opcode_name, operands);
		return insn->infos();
	}
...
#define IDO_VSIZE(s, x, y, z, w) if ((s)&0x00008000) { if ((s)&0x00000080) { w; } else { z; } } else { if ((s)&0x00000080) { y; } else { x; } }
#define IDO(x) return ((AllegrexInstruction *)&Allegrex::x)
    AllegrexInstruction *DecodeInstruction(u32 opcode)
    {
        switch ((opcode >> 26) & 0x0000003f)
        {
#include "al_decoder.h"
        case 39: return AllegrexBreakpoint::DecodeOpcode(opcode); // emulator specific to allow breakpoints
        }
        IDO(reserved_instruction);
    }
#undef IDO

#define IDO(x) do { AllegrexInstruction_##x::FastInterpret(context, opcode); return; } while (0)
    void FastInterpret(AlContext &context, u32 opcode)
    {
        switch ((opcode >> 26) & 0x0000003f)
        {
#include "al_decoder.h"
        case 39: AllegrexBreakpoint::DecodeOpcode(opcode)->Interpret(context, opcode); return; // emulator specific to allow breakpoints
        }
...
#undef IDO_VSIZE
	...
	{
        Function currentFunction = { startAddr, 0 };
		...
		for (address = startAddr; address <= endAddr; address += 4)
		{
			...
			u32 opcode = Memory::memw(address);
			...
			{
				target = address + (AllegrexInstruction::simm16(opcode) << 2);
			...
			if (looking)
			{
				if (address >= furthest_branch)
				{
					...
					{
						target = (address & 0xF0000000)|(AllegrexInstruction::uimm26(opcode) << 2);
...
static std::map< u32, AllegrexBreakpoint > breakpoints;
static u32 step_address = 0;

void AllegrexBreakpoint::Interpret(AlContext &context, u32 opcode)
{
	if (address)
	{
		if (address == step_address)
		{
			step_address = 0;
			insn->Interpret(context, this->opcode);
		}
		else
		{
			context.pc  = address;
			context.npc = address + 4;
		}
		if (temporary)
		{
			if (count-- == 1)
			{
				Memory::memw(address) = this->opcode;
				breakpoints.erase(opcode);
			}
		}
	}
}

void AllegrexBreakpoint::Disassemble(u32 address, u32 opcode, char *opcode_name, char *operands, char *comment)
{
	if (insn)
	{
		insn->Disassemble(address, this->opcode, opcode_name, operands, comment);
	}
	else
	{
		::strcpy(opcode_name, "brkpt");
		::strcpy(operands, "");
		::strcpy(comment, "Unexpected Breakpoint");
	}
}

u64 AllegrexBreakpoint::infos()
{
	return (insn) ? insn->infos() : Allegrex::IS_UNIMPLEMENTED;
}

AllegrexBreakpoint *AllegrexBreakpoint::DecodeOpcode(u32 opcode)
{
	std::map< u32, AllegrexBreakpoint >::iterator i = breakpoints.find(opcode);

	if (i != breakpoints.end())
		return &i->second;

	return &AllegrexBreakpoint::self();
}
