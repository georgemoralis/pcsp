...
__forceinline float signum(float i)
{
    int &v = *((int *)&i);
    return v ? ((v & 0x80000000) ? -1.0f : 1.0f) : 0.0f;
}
// VFPU stuff
...
template< int vsize > void doVCMP(int vs, int vt, int cond)
{
    ...
    else
    {
        ...
        for (int i = 0; i < vsize; ++i)
        {
            ...
            {
                cc = (((cond & 1) == 1) && _isnan(v1[i])) ||
                    (((cond & 2) == 2) && !_finite(v1[i]));
                if ((cond & 4) == 4)
                {
                    cc = !cc;
                }
...
// VFPU4:VCMOVT
template< int vsize > void doVCMOVT(int imm3, int vd, int vs)
{
    if (imm3 < 6)
    {
        if (vcr.cc[imm3])
...
// VFPU4:VCMOVF
template< int vsize > void doVCMOVF(int imm3, int vd, int vs)
{
    if (imm3 < 6)
    {
        if (!vcr.cc[imm3])
